...
import { Globe, Plus, Trash2, ExternalLink, Loader2, Search, AlertCircle } from 'lucide-react';
...
const Projects: React.FC<ProjectsProps> = ({ projects, selectedProjectId, setSelectedProjectId, user }) => {
  ...
  const handleAddProject = async (e: React.FormEvent) => {
    ...
    setIsAdding(true);
    try {
      const res = await fetch(`/api/gitlab/project?projectId=${newProjectId}`);
            ...
            >
              <div className="p-8">
                ...
                <form onSubmit={handleAddProject} className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-zinc-500 uppercase mb-2">GitLab Project ID</label>
                    <input
                      type="text"
                      value={newProjectId}
                    ...
                    <p className="mt-2 text-[10px] text-zinc-600">
                      Enter the numeric Project ID from GitLab.
                    </p>
                  </div>
                  
                  <button
                    type="submit"
                    disabled={isAdding || !newProjectId.trim()}
